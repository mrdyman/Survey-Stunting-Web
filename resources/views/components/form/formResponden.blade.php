<form id="{{$form_id}}" action="#" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            {{-- Kartu Keluarga --}}
            @component('components.formGroup.input', ['label' => 'Nomor Kartu Keluarga', 'type' => 'text', 'class' =>
            'angka', 'id' => 'kartu-keluarga', 'name' => 'kartu_keluarga', 'placeholder' => 'Masukkan', 'value' =>
            $responden->nomor_hp ?? null])
            @endcomponent
        </div>
    </div>

    {{-- alamat --}}
    @component('components.formGroup.textArea', ['label' => 'Alamat', 'class' => '', 'id' => 'alamat', 'name' =>
    'alamat', 'placeholder' => 'Masukkan', 'value' => $responden->alamat ?? null])
    @endcomponent
    ...
    <div class="row">
        <div class="col-lg col-md">
            {{-- Nomor HP --}}
            @component('components.formGroup.input', ['label' => 'Nomor HP (Optional)', 'type' => 'text', 'class' =>
            'angka', 'id' => 'nomor-hp', 'name' => 'nomor_hp', 'placeholder' => 'Masukkan', 'value' =>
            $responden->nomor_hp ?? null])
            @endcomponent
        </div>
    </div>
    ...
    $(function () {
        ...
        $("#provinsi").change(function () {
            if ($("#provinsi").val() != '') {
                ...
                fetch(`http://www.emsifa.com/api-wilayah-indonesia/api/regencies/` + $("#provinsi")
                        .val() + `.json`)
                    .then(response => response.json())
        ...
        $("#kabupaten-kota").change(function () {
            if ($("#kabupaten-kota").val() != '') {
                ...
                fetch(`http://www.emsifa.com/api-wilayah-indonesia/api/districts/` + $(
                        "#kabupaten-kota").val() + `.json`)
                    .then(response => response.json())
        ...
        $("#kecamatan").change(function () {
            if ($("#kecamatan").val() != '') {
                ...
                fetch(`http://www.emsifa.com/api-wilayah-indonesia/api/villages/` + $("#kecamatan")
                        .val() + `.json`)
                    .then(response => response.json())
